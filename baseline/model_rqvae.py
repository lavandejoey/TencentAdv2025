    ...
    def _create_codebook(self, data):
        if self.kmeans_method == 'kmeans':
            ...
            BKmeans = BalancedKmeans(
                num_clusters=int(self.num_clusters), kmeans_iters=self.kmeans_iters, tolerance=1e-4, device=self.device
            )
            _codebook, _ = BKmeans.fit(data)
        else:
            _codebook = torch.randn(int(self.num_clusters), self.codebook_emb_dim)
